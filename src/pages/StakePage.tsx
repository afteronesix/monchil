...
export function StakePage() {
        ...
        keys.forEach(key => {
            ...
            nft.ids.forEach(() => {
                ...
                if (nft.isOld) {
                    ...
                    dailyRate = parseFloat(formatEther(toBig(data[rateIdx])));
                    rateIdx++;
                }
                dataIdx += 4;
            });
    ...
    const renderActionPanel = () => {
        if (!isConnected) return <p className="text-gray-400">Connect your wallet to see staking options.</p>;
        ...
        return (
            <div className="bg-gray-700 p-4 rounded-xl text-left">
                ...
                <div className="text-sm bg-gray-800 p-3 rounded-lg mb-4">
                    <p className="text-gray-400">Daily Reward per NFT: <span className="font-bold text-yellow-400 ml-1">{data.dailyRate.toFixed(2)} $MON</span></p>
                </div>
                <div className="grid grid-cols-3 gap-3 text-center mb-6">
                    ...
                    <div className="bg-gray-800 p-3 rounded-xl"><p className="text-gray-400 text-xs">Pending Reward</p><p className="text-yellow-400 text-lg font-bold">{data.reward.toFixed(4)} $MON</p></div>
                </div>
                {!data.approved ? (
                    <button onClick={async () => {
                        const nftAbi = selectedNft.isOld ? abiNFT : monchilIdAbi;
                        await writeContractAsync({ abi: nftAbi, address: selectedNft.contract, functionName: "setApprovalForAll", args: [selectedNft.staking, true] });
                        refetch();
                    }} className="w-full bg-yellow-500 hover:bg-yellow-600 text-black py-3 rounded-xl font-bold transition">Approve NFT for Staking</button>
                ) : (
                    <>
                        <button onClick={callClaim} disabled={!rewardValid} className={`w-full py-2 rounded-xl font-bold mb-4 transition ${rewardValid ? 'bg-green-500 hover:bg-green-600 text-white' : 'bg-gray-500 text-gray-300 cursor-not-allowed'}`}>Claim {data.reward.toFixed(4)} $MON</button>
                        <div className="flex gap-2 mb-4">
                            <input type="number" min={1} value={stakeAmount} onChange={(e) => setStakeAmount(Math.max(1, parseInt(e.target.value || "1")))} className="w-20 bg-gray-800 text-white p-2 rounded-lg text-center" />
                            <button onClick={() => callStake(stakeAmount)} disabled={!stakeAmountValid} className={`flex-1 py-2 rounded-xl font-bold transition ${stakeAmountValid ? 'bg-purple-600 hover:bg-purple-700 text-white' : 'bg-gray-500 text-gray-300 cursor-not-allowed'}`}>Stake</button>
                        </div>
                        <div className="flex gap-2">
                            <input type="number" min={1} value={unstakeAmount} onChange={(e) => setUnstakeAmount(Math.max(1, parseInt(e.target.value || "1")))} className="w-20 bg-gray-800 text-white p-2 rounded-lg text-center" />
                            <button onClick={() => callUnstake(unstakeAmount)} disabled={!unstakeAmountValid} className={`flex-1 py-2 rounded-xl font-bold transition ${unstakeAmountValid ? 'bg-red-500 hover:bg-red-600 text-white' : 'bg-gray-500 text-gray-300 cursor-not-allowed'}`}>Unstake</button>
                        </div>
                    </>
                )}
            </div>
    ...
    return (
        <div className="flex flex-col items-center min-h-screen p-4">
            <div className="bg-gray-900 border-purple-700 rounded-2xl shadow-lg p-6 max-w-2xl w-full text-center mx-auto">
                <h1 className="text-4xl font-bold text-pink-600 mb-6">Monchil Staking Center</h1>
